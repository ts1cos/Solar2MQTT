...
bool onPIGS() //QALL<cr>: Device general status parameters inquiry
{
  //if(_commandBuffer[1]=='N') _qAv.QALL = false; //deactivate if NAK
  ...
  {
    _qRaw.QALL = _commandBuffer;
    int index = 1; //after the starting '('
  _qallMessage.gridV =                   getNextFloat(_commandBuffer, index);//1
  _qallMessage.gridHz =                  getNextFloat(_commandBuffer, index);//2
  _qallMessage.acOutV =                  getNextFloat(_commandBuffer, index);//3
  _qallMessage.acOutHz =                 getNextFloat(_commandBuffer, index);//4
  _qallMessage.acOutW =            (short)getNextLong(_commandBuffer, index);//6
  _qallMessage.acOutPercent =       (byte)getNextLong(_commandBuffer, index);//7
  _qallMessage.battV =                   getNextFloat(_commandBuffer, index);//9
  _qallMessage.battPercent =        (byte)getNextLong(_commandBuffer, index);//10
  _qallMessage.battChargeA =        (byte)getNextLong(_commandBuffer, index);//11
  _qallMessage.battDischargeA =     (byte)getNextLong(_commandBuffer, index);//12
  _qallMessage.solarV =             (byte)getNextLong(_commandBuffer, index);//13
  _qallMessage.solarA =             (byte)getNextLong(_commandBuffer, index);//14
  _qallMessage.solarW =                getNextFloat(_commandBuffer, index);//15
 // _qallMessage.daily_pv_power_generated =                getNextFloat(_commandBuffer, index);//16
 // _qallMessage.total_pv_power_generated =    getNextLong(_commandBuffer, index);//17
 // _qallMessage.device_mode =         getNextLong(_commandBuffer, index);//18
 // _qallMessage.warning_code =    getNextFloat(_commandBuffer, index);//19
 // _qallMessage.fault_code = getNextFloat(_commandBuffer, index);//20

    //Beta
    //calculate the real SOC
    if (_qallMessage.sccBattV > _qallMessage.battV)
    {
      _qallMessage.cSOC = constrain(round(mapf(_qallMessage.battV, _qpiriMessage.battreChargeV, _qpiriMessage.battBulkV, 0, 100)),0,100);
    }
    else
    {
      _qallMessage.cSOC = constrain(round(mapf(_qallMessage.battV, _qpiriMessage.battreChargeV, _qpiriMessage.battFloatV, 0, 100)),0,100);
    }
    return true;
  }
}
...
void requestInverter(qCommand com)
{
  ...
  case qCommand::QALL: if(sendCommand("QALL")) onPIGS(); break; //vervollständigen!
